...
class Move():
    ...
    def apply(self, board):
        ...
        new_x = self.piece.x / 81.25
        new_y = self.piece.y / 81.25
        board[int(checker_x), int(checker_y)].checker = None
        board[int(new_x), int(new_y)].checker = deepcopy(self.checker)
        board[int(new_x), int(new_y)].checker.x = board[int(new_x), int(new_y)].x / 81.25
        board[int(new_x), int(new_y)].checker.y = board[int(new_x), int(new_y)].y / 81.25

        for piece in self.jumped:
            board[int(piece.x / 81.25), int(piece.y / 81.25)].checker = None
        return board
...
def findMoves(board, color):
    ...
    for piece in board.flat:
        ...
        if piece.checker.king:
            pieces = findNeighbor(board, piece.x / 81.25, piece.y / 81.25)
        elif color:
            pieces = findNeighbor(board, piece.x / 81.25, piece.y / 81.25, down=True)
        elif not color:
            pieces = findNeighbor(board, piece.x / 81.25, piece.y / 81.25, up=True)
        for new_piece in pieces:
            dirs = []
            if color is True or piece.checker.king:
                dirs.append(checkNeighbor(new_piece.x / 81.25, new_piece.y / 81.25, piece.x / 81.25, piece.y / 81.25,
                                          down=True))
            if color is False or piece.checker.king:
                dirs.append(
                    checkNeighbor(new_piece.x / 81.25, new_piece.y / 81.25, piece.x / 81.25, piece.y / 81.25, up=True))
    ...
    for piece in board.flat:
        ...
        for new_piece in pieces:
            ...
            if new_piece.checker is None or new_piece.checker.black == color:
                continue
            if color is True or piece.checker.king:
                dir.append(checkNeighbor(new_piece.x / 81.25, new_piece.y / 81.25, piece.x / 81.25, piece.y / 81.25,
                                         down=True))
            if color is False or piece.checker.king:
                dir.append(
                    checkNeighbor(new_piece.x / 81.25, new_piece.y / 81.25, piece.x / 81.25, piece.y / 81.25, up=True))
        ...
        for option in options:
            new_piece = None
            if option.x / 81.25 == 0 or option.x / 81.25 == 7 or option.y / 81.25 == 0 or option.y / 81.25 == 7:
                x += 1
                continue
            if dirs[x] == 0:
                new_piece = board[int(option.x / 81.25) + 1, int(option.y / 81.25) + 1]
                x += 1
            elif dirs[x] == 1:
                new_piece = board[int(option.x / 81.25 - 1), int(option.y / 81.25 + 1)]
                x += 1
            ...
            if new_piece.checker is None:
                ...
                new_board = Board.copyBoard(board)
...
# Weighs a board based different types of avMinmaxlable moves
def weighBoard(board):
    ...
    for move in white_moves:
        if doesMoveProtect(board, move, False):
            ...
            move.weight = 81.25 + move.distance - 1
...
# Checks f a move is a suicide run for a checker
# Improves Minmax significantly
def enemyJump(board, move, color):
    enemy_jumps = findJumps(move.apply(Board.copyBoard(board)), not color)
...
def doesMoveKing(board, move, color):
    if color:
        if move.piece.x / 81.25 == 7:
            return True
    elif not color:
        if move.piece.x / 81.25 == 0:
            return True
...
def doesMoveEscape(board, move, color):
    ...
    for jump in enemy_jumps:
        for victim in jump.jumped:
            if victim.x / 81.25 == move.checker.x and victim.y / 81.25 == move.checker.y:
                return True
    return False
...
def minimax(depth, color, board, a, b):
    ...
    if color:
        ...
        for move in black_moves:
            if move.type == "Jump":
                # The Minmax has to jump
                return move

        # Evaluates future impact of moves and ranks them accordingly
        for move in black_moves:
            copy = Board.copyBoard(board)
            val = minimax(depth + 1, False, move.apply(copy), a, b)
            if (best_move is None or val.weight < best_move.weight) and val is not None:
                best_move = val
        ...
        # Evaluates future impact of moves and ranks them accordingly
        for move in white_moves:
            val = minimax(depth + 1, True, move.apply(Board.copyBoard(board)), a, b)
            if (best_move is None or val.weight > best_move.weight)  and val is not None:
                best_move = val
            if best_move is not None:
...
# Does the work of computing what move to do next based on random value
def random(color, board):
    white_moves = findMoves(board, False) + findJumps(board, False)
    black_moves = findMoves(board, True) + findJumps(board, True)
    if color:
        # Returns move random for black
        r = randint(0, len(black_moves)-1)
        return black_moves[r]
    else:
        # Returns move random for white
        r = randint(0, len(white_moves)-1)
        return white_moves[r]
